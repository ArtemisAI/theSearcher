...
class TestTheSearcherIntegration(unittest.TestCase):
    ...
    def test_script_runs_dry_run(self):
        ...
        # Updated expected output for dry run
        self.assertIn("Dry run: Would search for images related to 'test_integration' using Google Search.", process.stdout)
        self.assertIn("Result: Dry run: Searched for 'test_integration' via Google", process.stdout)
    ...
    def test_script_runs_live_run(self):
        ...
        self.assertEqual(process.returncode, 0, f"Process failed with error: {process.stderr}")
        # Updated expected output for live run
        self.assertIn("Live run: Searching for images related to 'test_integration_live' using Google Search...", process.stdout)
        self.assertIn("Live run: Found image URL: https://example.com/image_for_test_integration_live.jpg", process.stdout)
